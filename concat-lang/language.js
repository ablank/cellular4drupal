...
function parseDoc(src, options) {
  ...
  exports.openTag = function (options) {
    ...
    console.log('Processing as ' + options.type);
  ...
  exports.format = function (filepath, src, options) {
    ...
    console.log('Looking for ' + options.type + ' tags in ' + filepath);
...
module.exports = function parselang(src, options) {
  new Promise((res, rej) => {
    let parsed = parseDoc(src, options);
    let output;
  
    if (parsed) {
      output = parsed.then(res).catch(rej);
    } 
  });
};
