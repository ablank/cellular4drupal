!function() {
    ...
    function d3_map_empty() {
        for (var key in this) if (key.charCodeAt(0) === d3_map_prefixCode) return !1;
        return !0;
    }
    function d3_Set() {}
    ...
    function d3_lab(l, a, b) {
        return this instanceof d3_lab ? (this.l = +l, this.a = +a, void (this.b = +b)) : arguments.length < 2 ? l instanceof d3_lab ? new d3_lab(l.l, l.a, l.b) : l instanceof d3_hcl ? d3_hcl_lab(l.l, l.c, l.h) : d3_rgb_lab((l = d3_rgb(l)).r, l.g, l.b) : new d3_lab(l, a, b);
    }
    ...
    function d3_rgb_parse(format, rgb, hsl) {
        ...
        return (color = d3_rgb_names.get(format)) ? rgb(color.r, color.g, color.b) : (null == format || "#" !== format.charAt(0) || isNaN(color = parseInt(format.substring(1), 16)) || (4 === format.length ? (r = (3840 & color) >> 4, 
    ...
    function d3_locale_numberFormat(locale) {
        var locale_decimal = locale.decimal, locale_thousands = locale.thousands, locale_grouping = locale.grouping, locale_currency = locale.currency, formatGroup = locale_grouping ? function(value) {
            for (var i = value.length, t = [], j = 0, g = locale_grouping[0]; i > 0 && g > 0; ) t.push(value.substring(i -= g, i + g)), 
            g = locale_grouping[j = (j + 1) % locale_grouping.length];
            return t.reverse().join(locale_thousands);
        } : d3_identity;
        return function(specifier) {
            var match = d3_format_re.exec(specifier), fill = match[1] || " ", align = match[2] || ">", sign = match[3] || "", symbol = match[4] || "", zfill = match[5], width = +match[6], comma = match[7], precision = match[8], type = match[9], scale = 1, prefix = "", suffix = "", integer = !1;
            ...
            return function(value) {
                ...
                return zcomma && (before = formatGroup(padding + before)), negative += prefix, value = before + after, 
                ("<" === align ? negative + value + padding : ">" === align ? padding + negative + value : "^" === align ? padding.substring(0, length >>= 1) + negative + value + padding.substring(length) : negative + (zcomma ? value : padding + value)) + fullSuffix;
            };
        };
    }
    ...
    function d3_interpolateString(a, b) {
        ...
        for (a += "", b += ""; (am = d3_interpolate_numberA.exec(a)) && (bm = d3_interpolate_numberB.exec(b)); ) (bs = bm.index) > bi && (bs = b.substring(bi, bs), 
        s[i] ? s[i] += bs : s[++i] = bs), (am = am[0]) === (bm = bm[0]) ? s[i] ? s[i] += bm : s[++i] = bm : (s[++i] = null, 
        q.push({
            i: i,
            x: d3_interpolateNumber(am, bm)
        })), bi = d3_interpolate_numberB.lastIndex;
        return bi < b.length && (bs = b.substring(bi), s[i] ? s[i] += bs : s[++i] = bs), 
        s.length < 2 ? q[0] ? (b = q[0].x, function(t) {
            return b(t) + "";
        }) : function() {
            return b;
    ...
    function d3_scale_quantile(domain, range) {
        ...
        var thresholds;
        return scale.domain = function(x) {
            return arguments.length ? (domain = x.filter(d3_number).sort(d3_ascending), rescale()) : domain;
        }, scale.range = function(x) {
            return arguments.length ? (range = x, rescale()) : range;
    ...
    d3.ascending = d3_ascending, d3.descending = function(a, b) {
        ...
        if (1 === arguments.length) for (;++i < n; ) d3_number(a = array[i]) ? s += a : --j; else for (;++i < n; ) d3_number(a = f.call(array, array[i], i)) ? s += a : --j;
        return j ? s / j : void 0;
        ...
        array.length ? d3.quantile(array.sort(d3_ascending), .5) : void 0;
    };
    ...
    d3.interpolateZoom = function(p0, p1) {
        ...
        function touchstarted() {
            ...
            var scale0, that = this, dispatch = event.of(that, arguments), locations0 = {}, distance0 = 0, zoomName = ".zoom-" + d3.event.changedTouches[0].identifier, touchmove = "touchmove" + zoomName, touchend = "touchend" + zoomName, targets = [], subject = d3.select(that).on(mousedown, null).on(touchstart, started), dragRestore = d3_event_dragSuppress();
            d3_selection_interrupt.call(that), started(), zoomstarted(dispatch);
        }
